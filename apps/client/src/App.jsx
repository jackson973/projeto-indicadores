...
const App = () => {
  ...
  return (
    <Flex minH="100vh">
      ...
      >
        {/* Header */}
        <Flex align="center" justify={sidebarOpen ? "space-between" : "center"} px={sidebarOpen ? 5 : 0} py={4} minH="64px">
          {sidebarOpen && (
            <Box>
        ...
        {/* Navigation items */}
        <VStack spacing={1} align="stretch" px={sidebarOpen ? 3 : 2} py={4} flex={1}>
          {navItems.filter((item) => item.show).map((item) => (
            <Tooltip key={item.view} label={item.label} placement="right" isDisabled={sidebarOpen}>
              <Box
                as="button"
                display="flex"
                alignItems="center"
                ...
                onClick={() => {
                  if (!item.disabled) setActiveView(item.view);
                }}
                textAlign="left"
                w="full"
                whiteSpace="nowrap"
        ...
        {/* Bottom actions */}
        <VStack spacing={1} align="stretch" px={sidebarOpen ? 3 : 2} py={3}>
          <Tooltip label={colorMode === "light" ? "Modo escuro" : "Modo claro"} placement="right" isDisabled={sidebarOpen}>
            <Box
              as="button"
              display="flex"
              alignItems="center"
            ...
            >
              ...
              {sidebarOpen && (colorMode === "light" ? "Modo escuro" : "Modo claro")}
            </Box>
          </Tooltip>
          <Tooltip label="Sair" placement="right" isDisabled={sidebarOpen}>
            <Box
              as="button"
              display="flex"
              alignItems="center"
